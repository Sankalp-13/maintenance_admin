...
class AssignedJobCubit extends Cubit<AssignedJobState>{
  AssignedJobCubit(): super(AssignedJobLoadingState());
  final storage = const FlutterSecureStorage();
  AssignedRepo assignedJobsRepo = AssignedRepo();

  void assignedJobs() async{
    try{
      emit(AssignedJobLoadingState());
      String? token = await storage.read(key: "accessToken");
      List<AssignedJobResponse> assignedJobs = await assignedJobsRepo.assignedJobs(token!);
